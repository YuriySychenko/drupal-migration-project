 ...
 *
 * @MigrateSource(
 *   id = "migrate_i18_aphorism",
 *   source_module = "node"
 * )
 */
class MigrateI18nAphorism extends FieldableEntity {
  /**
   * The module handler.
   *
  ...
  public function prepareRow(Row $row) {
    ...
    foreach ($this->getFields('node', $type) as $field_name => $field) {
      ...
      // translatable.
      if ($field_name !== 'field_aphorism_author') {
  ...
  protected function handleTranslations(SelectInterface $query) {
    ...
    if (empty($this->configuration['translations'])) {
      // No translations: Yield untranslated node, or default translations.
